...
Определи подходящий заголовок на основе смысла.
Добавь YAML-метаданные: дату и теги.
Верни ТОЛЬКО содержимое файла .md без пояснений и кавычек.
"""
...
def _make_filename(text: str) -> str:
    safe_title = "".join(c for c in text[:30] if c.isalnum() or c in (" ", "-", "_"))
